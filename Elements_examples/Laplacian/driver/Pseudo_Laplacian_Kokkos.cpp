...
void calculate_ref_elem(){
    ...
    auto partial_xi = ViewCArray <real_t> (partial_xia, elem.num_verts);

    real_t partial_etaa[elem.num_verts];
    auto partial_eta = ViewCArray <real_t> (partial_etaa, elem.num_verts);

    real_t partial_mua[elem.num_verts];
    auto partial_mu = ViewCArray <real_t> (partial_mua, elem.num_verts);
    ...
    for(int node_rid = 0; node_rid < ref_elem.num_ref_nodes(); node_rid++){
        ...
        for(int basis_id = 0; basis_id < elem.num_basis; basis_id++){
            ...
            ref_elem.ref_nodal_gradient_(node_rid, basis_id, 0) = partial_xi(basis_id);
            ref_elem.ref_nodal_gradient_(node_rid, basis_id, 1) = partial_eta(basis_id);
            ref_elem.ref_nodal_gradient_(node_rid, basis_id, 2) = partial_mu(basis_id);
            ...
            // std::cout << "XX Partial Xi = "  << ref_elem.ref_elem_grad(node_rid, basis_id, 0) << std::endl;
            // std::cout << "XX Partial Eta = " << ref_elem.ref_elem_grad(node_rid, basis_id, 1) << std::endl;
            // std::cout << "XX Partial Mu = "  << ref_elem.ref_elem_grad(node_rid, basis_id, 2) << std::endl;
                    ...
                    for(int dim_j = 0; dim_j < mesh.num_dim(); dim_j++){
                        ...
                        for(int basis_id = 0; basis_id < mesh.num_nodes_in_cell(); basis_id++){
                            ...
                            mesh.node_jacobian(node_gid, dim_i, dim_j) += 
                                mesh.node_coords(0, ref_node_gid , dim_i) * ref_elem.ref_nodal_gradient(node_lid, basis_id, dim_j);
                        }
                    }
                }
            }
